...
import { SOLAR_TERM_DAYS } from '../data/solar-terms';
...
export function getSolarTerm(year: number, month: number, day: number): string | undefined {
  ...
  if (day === dates[termIndex + 1]) return SOLAR_TERMS[termIndex + 1];
  return undefined;
}

// 节气精确公历日期表（天文算法，覆盖1900-2100年）
export function getSolarTermDates(year: number): number[] {
  if (year < 1900 || year > 2100) {
    throw new Error('Year out of range');
  }
  return SOLAR_TERM_DAYS[year - 1900];
}
